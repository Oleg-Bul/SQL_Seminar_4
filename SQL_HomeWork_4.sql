...
WHERE MARK = 'LADA'
GROUP BY color, mark 
UNION SELECT MARK, color, COUNT(color) AS 'qty' FROM AUTO
WHERE MARK = 'BMW'
GROUP BY color, mark;

-- 2
SELECT DISTINCT mark, 
(SELECT COUNT(1) FROM auto a1 WHERE a1.mark = a.mark) AS qty,
(SELECT COUNT(1) FROM auto a1 WHERE a1.mark != a.mark) AS qty_of_another FROM auto a;
